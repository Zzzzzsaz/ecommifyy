...
export default function Wyniki({ user }) {
  ...
  const openAdd = (type, date) => { setDialog({ open: true, type, date }); setAmount(""); setDesc(""); };
  ...
  const getDayName = (dateStr) => DAYS_PL[new Date(dateStr + "T12:00:00").getDay()];
  ...
  return (
    <div className="p-4 pb-24 animate-fade-in" data-testid="wyniki-page">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <Badge variant="secondary" className="text-xs" data-testid="wyniki-user-badge">{user.name}</Badge>
      </div>

      {/* Shop Tabs */}
      <div className="flex gap-2 mb-5 overflow-x-auto pb-1" data-testid="wyniki-shop-tabs">
        {SHOPS.map((s) => (
          <button key={s.id} onClick={() => setShop(s.id)}
            ...
            data-testid={`wyniki-shop-tab-${s.id}`}
          >{s.name}</button>
        ))}
      </div>

      {/* Month Nav */}
      <div className="flex items-center justify-center gap-4 mb-6" data-testid="wyniki-month-nav">
        <Button variant="ghost" size="icon" onClick={prevMonth} className="text-ecom-muted hover:text-white" data-testid="wyniki-prev-month"><ChevronLeft size={20} /></Button>
        <span className="font-heading text-lg font-semibold text-white min-w-[160px] text-center">{MONTHS_PL[month - 1]} {year}</span>
        <Button variant="ghost" size="icon" onClick={nextMonth} className="text-ecom-muted hover:text-white" data-testid="wyniki-next-month"><ChevronRight size={20} /></Button>
      </div>
      ...
      {loading ? (
        ...
        <>
          {/* KPI Cards */}
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6" data-testid="wyniki-kpi-cards">
            {kpis.map((kpi, i) => (
              <Card key={i} className="bg-ecom-card border-ecom-border" style={i === 0 ? { borderTop: `2px solid ${shopColor}` } : {}}>
                <CardContent className="p-4">
                  ...
                  <p className={`text-lg font-heading font-bold tabular-nums mt-1 ${kpi.color}`} data-testid={`wyniki-kpi-${kpi.label.toLowerCase()}`}>{kpi.value}</p>
                </CardContent>
              </Card>
            ))}
          </div>
          ...
          <div className="space-y-2" data-testid="wyniki-daily-list">
            {stats?.days?.map((day) => {
              const hasData = day.income > 0 || day.ads > 0;
              return (
                <div key={day.date} className={`day-card rounded-lg border p-3 ${hasData ? "bg-ecom-card border-ecom-border" : "bg-ecom-card/50 border-ecom-border/50"}`} data-testid={`wyniki-day-${day.date}`}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex gap-1.5">
                      <button onClick={() => openAdd("income", day.date)} className="btn-press flex items-center gap-1 text-[10px] font-medium text-ecom-success bg-ecom-success/10 hover:bg-ecom-success/20 px-2 py-1 rounded-md" data-testid={`wyniki-add-income-${day.date}`}>
                        <Plus size={12} /> Przychod
                      </button>
                      <button onClick={() => openAdd("expense", day.date)} className="btn-press flex items-center gap-1 text-[10px] font-medium text-ecom-danger bg-ecom-danger/10 hover:bg-ecom-danger/20 px-2 py-1 rounded-md" data-testid={`wyniki-add-expense-${day.date}`}>
                        <Plus size={12} /> Ads
                      </button>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1">
                    <div><span className="text-ecom-muted text-[10px]">Przychod</span><p className="text-white text-sm tabular-nums font-medium">{fmtPLN(day.income)}</p></div>
                    <div><span className="text-ecom-muted text-[10px]">Ads</span><p className="text-ecom-danger text-sm tabular-nums font-medium">{fmtPLN(day.ads)}</p></div>
                    <div><span className="text-ecom-muted text-[10px]">Netto</span><p className="text-ecom-muted text-sm tabular-nums">{fmtPLN(day.netto)}</p></div>
                    <div><span className="text-ecom-muted text-[10px]">Zysk</span><p className={`text-sm tabular-nums font-medium ${day.profit >= 0 ? "text-ecom-success" : "text-ecom-danger"}`}>{fmtPLN(day.profit)}</p></div>
                  </div>
                </div>
              );
            })}
